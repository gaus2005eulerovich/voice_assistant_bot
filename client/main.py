...
import sys
# 1. Получаем путь к папке проекта (на уровень выше, чем client)
current_dir = os.path.dirname(os.path.abspath(__file__))
project_root = os.path.dirname(current_dir)

# 2. Добавляем этот путь в "мозги" Питона
sys.path.append(project_root)

# 3. ОТЛАДКА: Проверяем, видит ли он файл (выведет в консоль)
config_path = os.path.join(project_root, 'config.py')
print(f"🔍 Ищу конфиг здесь: {config_path}")
if os.path.exists(config_path):
    print("✅ Файл config.py НАЙДЕН!")
else:
    print("❌ Файл config.py НЕ НАЙДЕН. Проверь название файла!")

# 4. Теперь пробуем импортировать
try:
    from config import PERPLEXITY_API_KEY, TOKEN
